...
TcpServer::~TcpServer()
{
    for (auto &item : connections_)
    {
        ...
        item.second.reset();
        conn->getLoop()->runLoopCallback(std::bind(&TcpConnection::connDestory, conn));
    }
}

void TcpServer::setThreadPoolSize(int num)
{
    thread_pool_->setPoolSize(num);
}
...
{
    loop_->runLoopCallback(std::bind(&TcpServer::removeConnectionInLoop, this, conn_ptr));
}
...
void TcpServer::newConnection(int sockfd, const InetAddress &peer_addr)
{
    ...
    conn_ptr->setCloseCallback(std::bind(&TcpServer::removeConnection, this, std::placeholders::_1));

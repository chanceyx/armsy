...
EpollPoller::EpollPoller(EventLoop *loop)
    : Poller(loop), epollfd_(::epoll_create1(EPOLL_CLOEXEC)), events_(kDefaultChannelListSize)
{
    if (epollfd_ < 0)
    {
...
Timestamp EpollPoller::poll(int timeout, ChannelList *activeChannels)
{
    ...
    int num_events = ::epoll_wait(epollfd_, &*events_.begin(), static_cast<int>(events_.size()), timeout);
    int err = errno;
...
void EpollPoller::updateChannel(Channel *channel)
{
    ...
    LOG_INFO("finc=%s => fd=%d events=%d state=%d\n", __FUNCTION__, channel->fd(), channel->events(), st);
